    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-4 mb-4">
        <span className="text-xs font-black uppercase tracking-[0.3em] text-white/50 font-display flex items-center gap-2">
          ...
          {t('common.educationCard.education_certifications')}
        </span>
      ...
      <div className="relative z-10 flex-1 min-h-[0px] flex flex-col justify-center">
        ...
        <div className="relative w-full h-full">
          ...
          >
            {education.map((edu) => {
              const degree = language === 'fr' ? edu.degreeFr : edu.degreeEn;
              const period = language === 'fr' ? edu.periodFr : edu.periodEn;
              const skills = language === 'fr' ? edu.skillsFr : edu.skillsEn;
              return (
                <div key={edu.id} className="flex gap-4 sm:gap-5 items-center group/item hover:bg-white/5 p-3 -mx-3 rounded-2xl transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-bold text-white mb-0.5 truncate">{degree}</h4>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {skills.length > 4 && (
                        <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-md text-white/40">+{skills.length - 4}</span>
                      )}
                    </div>
                  </div>
                </div>
